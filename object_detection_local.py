...
def appended_filename(file):
    filename = file.split(".")[0]
    extension = file.split(".")[1]
    return f"{filename}-local.{extension}"
...
def draw_rectangles(file, data):
    ...
    for i, obj in data.iterrows():
        ...
        imgDraw.text(
            (obj["xmin"] + outlineWidth, obj["ymin"] + outlineWidth),
            label,
            fill="white",
            font=font,
        )
        ...
        imgDraw.rectangle(outlineShape, fill=None, outline=color, width=outlineWidth)

    # Save the new image to OUTPUT_PATH
    img.save(os.path.join(OUTPUT_PATH, appended_filename(file)))
...
def main():
    ...
    for file in files:
        ...
        output.append(
            {
                "file": appended_filename(file),
                "data": results.pandas().xyxy,
                "stats": stats,
            }
        )

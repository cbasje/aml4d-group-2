...
def object_det():
    try:
        ...
        local = request.args.get("local", type=bool)

        if local:
            data = object_detection_local.main()
        else:
...
def get_files(folder, name=None):
    ...
    if name is not None:
        ...
        return render_template("files.html", folder=folder, files=sorted(files))
